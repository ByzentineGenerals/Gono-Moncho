...
contract SubscriptionProtocolTest is Test {
    ...
    function testRegisterCreator() public {
        vm.prank(creator1);
        protocol.registerCreator(
            "Investigative Reporter",
            "Daily investigative journalism"
        );

        (, string memory name, , , bool isActive, ) = protocol.creators(
            creator1
        );
    ...
    function testCreateSubscriptionPlan() public {
        ...
        (, , uint256 monthlyPrice, uint256 annualPrice, ) = protocol.plans(
            creator1,
            SubscriptionProtocol.SubscriptionTier.BASIC
        );
    ...
    function testSubscribeMonthly() public {
        ...
        (bool isActive, , , ) = protocol.getSubscriptionStatus(
            subscriber1,
            creator1
        );
        assertTrue(isActive);
    ...
    function testSubscribeAnnual() public {
        ...
        (, , uint256 expiryDate, ) = protocol.getSubscriptionStatus(
            subscriber1,
            creator1
        );
    ...
    function testPublishPremiumContent() public {
        ...
        (, , SubscriptionProtocol.SubscriptionTier minTier, , ) = protocol
            .premiumContent("premium-article-001");

        assertEq(
            uint8(minTier),
            uint8(SubscriptionProtocol.SubscriptionTier.BASIC)
        );
    }
